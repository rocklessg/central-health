...
public class MedicalServicesController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateMedicalService([FromBody] CreateMedicalServiceRequest request, CancellationToken cancellationToken)
    {
        ...
        return result.Success ? CreatedAtAction(nameof(GetMedicalService), new { id = result.Data!.Id }, result) : BadRequest(result);
    }
    ...
    {
        var result = await _medicalServiceService.GetMedicalServiceByIdAsync(id, cancellationToken);
    ...
    {
        var result = await _medicalServiceService.DeleteMedicalServiceAsync(id, cancellationToken);
        return result.Success ? Ok(result) : BadRequest(result);
    }
}
